...
class NoizeBob_2D {
  ...
  NoizeBob_2D(float factor_, float inc_, float falloff_) {
    init(factor_, inc_, inc_ * random(1), falloff_);
  }
  ...
  float getBobtail(float incx, float incy) {
   noiseDetail(nload, falloff);

   return factor * noise(bob1 + incx * bob1Inc, bob2 + incy * bob2Inc);
  }
}
