...
//https://www.iquilezles.org/www/articles/palettes/palettes.htm  
  ...
  float a2 = 0.7;  // sat : center
  float a3 = 0.8;  // bright : center
  float b1 = 0.5; // hue: full oscillation
  float b2 = 0.2;  // sat needs to be clamped at 1.0
  float b3 = 0.2;  // bri needs to be clamped at 1.0
  float c1 = 1.0;
  float c2 = 1.0;
  float c3 = 1.0;
  float d1 = 0.;
 ...
 color getColor(float t0) {
  ...
  h1 = h1 - floor(h1);  // hue gets full oscillation
  
  float b2f = b2 * cos(TWO_PI*(c2*tnom+d2));

  float s2 = constrain(factor * (a2 + b2f), 0.0, 1.0);  // sat must be constrained
  
  float b3f = + b3 * cos(TWO_PI*(c3*tnom+d3));
  float b3 = constrain(factor * (a3 + b3f), 0.0, 1.0); // brit must be constrained
  c = color(h1,s2,b3);
  c = color(h1,s2,b3);
  return c;   
 }
