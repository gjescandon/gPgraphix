class FlockBox{
  ...
  void draw(){
    ...
    for (int i = fs; i >= 0; i--) {
      for (int j = 0; j <= fs; j++) {
        ...
        translate(xbob.getBobTail(i) - 5.0, ybob.getBobTail(j) - 5.0, -100);
        pushMatrix();
        float bobtail = nb2d.getBobTail(30.*i,30.*j);
        float bobtailC = 0.;
        if ((bobtail > 0.4) || (bobtail < 0.6)) {
          translate((i)*fsw, j*fsh, (1-0.3*cos(0.03*frameCount))*(50*bobtail));
          noStroke();
  ...
  void init(){
    nb2d = new NoizeBob_2D(1.0, 0.009, 0.7);  //  z noise
