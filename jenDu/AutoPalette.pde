...
//https://www.iquilezles.org/www/articles/palettes/palettes.htm  
 ...
 color getColor50(float t0){
   ...
   return color(hue(c), map(saturation(c),0.,1.,0.6,1.), map(brightness(c),0.,1.,0.6,1.), 0.5);
 }

 color getColor80(float t0){
   colorMode(HSB,1.0);
   color c = getColor(t0);  
   
   return color(hue(c), map(saturation(c),0.,1.,0.6,1.), map(brightness(c),0.,1.,0.6,1.), 0.8);
 }
