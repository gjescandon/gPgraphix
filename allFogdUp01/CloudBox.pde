...
QuilezFunctions qf;
 ...
 CloudBox(PImage img_) {
  ...
  void setup() {
    ...
    //img = loadImage("llama4_1280.jpg");
  }
  
  void draw() {
    colorMode(HSB,1.0);
    
    cloudbox = createImage(width,height, ARGB);
    ...
    img.mask(cloudbox);
    image(img, 0.01 * width, 0.01 * height, 0.98*width, 0.98*height);
  ...
  void loadCloudPixels(int mag) {
     ...
     for (int x = 0; x < cloudbox.width; x++) {
      ...
      for (int y = 0; y < cloudbox.height; y++) {
        ...
        bright *= qf.expSustainedImpulse(1.0*frameCount, 1000., 0.1);
        ...
        if (bright > duty) {
          ...
          cloudbox.pixels[x+y*cloudbox.width] = color(1.);
        }
      }
    } 
