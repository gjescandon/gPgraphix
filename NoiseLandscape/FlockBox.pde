class FlockBox{
  ...
  void draw(){
    ...
    ybob.getBob();
        ...
        translate(xbob.getBobTail(1) - 0.5*width, ybob.getBobTail(1) - 0.5*height,0);
    float cmin = 1.;
    float cmax = 0.;
    ...
    for (int i = fsx; i >= 0; i--) {
      for (int j = 0; j <= fsy; j++) {
        pushMatrix();
        translate((i)*fsw, j*fsh, (1-cos(0.006*frameCount))*(20*nb2d.getBobtail(30*i,30*j)));
  ...
  void init02(float brit, float myRange){
    apal = new AutoPalette(random(1), 1., brit);
    range = myRange;
  }
}
