class Spirograph {
 ...
 Spirograph(float r1_,float r2_,int n1_,int n2_, color sColor_, color fColor_, int delay_) {
  ...
  float[] getPoint(float tOff) {
     float[] ret = new float[2];
     
     float ss = ssMax;// * 0.5 * (1+sin(0.003*frameCount));

     float x = r1 * cos(n1 * tOff) + r2 * cos(n2 * tOff);// + r3 * cos(n3 * theta);
     float y = r1 * sin(n1 * tOff) + r2 * sin(n2 * tOff);// + r3 * sin(n3 * theta) ;
     //vertex(offx - ss * x, offy + ss * y);
     //vertex(ss * x,  ss * y);
     ret[0] = ss*x;
     ret[1] = ss*y;
